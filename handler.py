...
from qrcode_lok import qrcode_loker, delete_file_photo_user, create
...
async def process_hi6_command(message: Message):
    await bot.send_message(chat_id=message.chat.id, text=f"Привет {message.from_user.first_name},"
                                                         ...
                                                         "что бы создать qrcode воспользуйтесь командой\n"
                                                         "/create_qrcode, и после введите ссылку или текст")
...
async def handle_docs_photo(message: Message):
    ...
    await bot.send_message(message.from_user.id, text=f"{qrcode_loker(message.from_user.id)}")
